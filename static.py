...
def manga_add(data: dict):
    ...
    cur = con.cursor()
    a = cur.execute(f"SELECT mylist FROM manga WHERE id = {data.get('id')}").fetchall()
    if a and a[0][0]:
        a = True
    else:
        a = None
...
def manga_mylist_add(manga_id: int):
    wd = os.getcwd()
    con = sqlite3.connect(f'{wd}/Desu/data.db')
    cur = con.cursor()
    cur.execute(f"UPDATE manga SET mylist = True WHERE id = {manga_id};")
    con.commit()
    con.close()
...
def manga_mylist_get():
    wd = os.getcwd()
    con = sqlite3.connect(f'{wd}/Desu/data.db')
    cur = con.cursor()
    a = cur.execute(f"SELECT * FROM manga WHERE mylist not Null;").fetchall()
    con.commit()
    con.close()
...
def manga_mylist_check(manga_id: int) -> bool:
    wd = os.getcwd()
    con = sqlite3.connect(f'{wd}/Desu/data.db')
    cur = con.cursor()
    a = cur.execute(f"SELECT mylist FROM manga WHERE id = {manga_id};").fetchall()
    con.commit()
    con.close()
    if a and a[0][0]:
        return True
...
def manga_mylist_rem(manga_id: int):
    wd = os.getcwd()
    con = sqlite3.connect(f'{wd}/Desu/data.db')
    cur = con.cursor()
    cur.execute(f"UPDATE manga SET mylist = Null WHERE id = {manga_id};")
    con.commit()
    con.close()
