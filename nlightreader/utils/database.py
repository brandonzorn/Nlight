...
class Database:
    ...
    def _make_manga(manga_data: dict[str, Any]) -> Manga:
        content_id = str(manga_data["content_id"])
        ...
        if not isinstance(chapters, int):
            logger.warning(f"Chapters must be int got {type(chapters)}")
            chapters = 0
