...
class ShikimoriLib(ShikimoriBase, LibParser):
    ...
    def get_user(self):
        ...
        if whoami and whoami.status_code == 200:
            data = whoami.json()
            return User(
                data.get('id'), data.get('nickname'), data.get('avatar')
            )
...
class Auth:
    ...
    def fetch_token(self, code):
        try:
            self.client.fetch_token(
                URL_SHIKIMORI_TOKEN, code, client_secret=self.client_secret
            )
    ...
    def request(
        self, method, url, params=None, json=None, ignore_authorize=False
    ):
        if (
